...
// algoritmo de ordenação do tipo String para array de objetos.

public class SelectionSortAlgoritmo_3 {

	public static void main(String[] args) {
		Pessoa letra[] = { new Pessoa(2, "Donna"), new Pessoa(13, "Luiz"), new Pessoa(22, "Sam"), new Pessoa(51, "Ana"),
				new Pessoa(9, "Rosa") };
...
// penúltima posição, sendo representado ao final pela váriavel "menor" que registra seu valor.
		for (int posicao = 0; posicao < letra.length - 1; posicao++) {
			int menor = posicao;
...
// registrando um novo valor para a variável "menor" se for o caso.
			for (int contador = posicao + 1; contador < letra.length; contador++) {
				if (letra[contador].nome.compareTo(letra[menor].nome) < 0)
					menor = contador;
			}
...
// posição restando o último índice sem ser comparado uma vez que representa o maior valor.
			if (menor != posicao) {
				Pessoa aux = letra[menor];
				letra[menor] = letra[posicao];
				letra[posicao] = aux;
			}
		}
		for (int i = 0; i < letra.length; i++)
			System.out.println(letra[i].nome + " : " + letra[i].idade);
	}
}